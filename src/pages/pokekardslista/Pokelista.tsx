...
// Las siguientes importaciones son necesarias para que el Hook anidado funcione

// Asumimos que useFavActions y useAppDispatch están definidos/exportados correctamente en el hook.ts
...
const ListaPokes = () => {
    ...
    // ----------------------------------------------------
    // SOLUCIÓN: COMPONENTE ANIDADO PARA LLAMAR AL HOOK DE FORMA LEGAL
    // ----------------------------------------------------
    
    const PokemonCardItem: React.FC<{ pokemon: CardPokemon }> = ({ pokemon }) => {
        
        // ✅ CORRECTO: Llamada al hook en el nivel superior de un componente de función
        const { isFavorite, handleToggleFavorite } = useFavActions(pokemon);
        
        return (
            <Card key={pokemon.id} sx={{ maxWidth: 345, width: '100%' }}>
                <CardHeader
                    avatar={<Avatar sx={{ bgcolor: red[500] }} aria-label="pokemon-avatar">{pokemon.id}</Avatar>}
                ...
                {/* Llama al handler del componente padre */}
    ...
    // ----------------------------------------------------
    // JSX PRINCIPAL: Ahora usa el componente anidado en el map
    // ----------------------------------------------------
